...
(established, therefore spared) and asserts that arriving mounts nothing:
  ...
  1. a fresh (hard) load of Community: no frame, no host, no Cloudflare
     script, no mount crumb — and the board composer carries the .ts-slot
     that puts it under the focus net;
  ...
  4. a soft hop back to Community and a focus on the composer (the net):
     still nothing.
...
def main():
    ...
    with Flow(port=9597) as f:
        ...
        # 1. a fresh open of the app, landing on Community
        ...
        checks.append(('fresh Community: the composer is under the net (.ts-slot)', st['slot']))
        ...
        # 4. back to Community softly, and the focus net on the composer
        n0 = len(ring(f))
        f.js("window.mcNav('community.html'); return 1;")
        f.wait("!!document.querySelector('.comment-form .comment-text')", timeout=20)
        f.js("var t=document.querySelector('.comment-form .comment-text'); if(t) t.focus(); return 1;")
        time.sleep(3)
        st = state(f)
        cr = turnstile_crumbs(f, n0)
        checks.append(('Community (soft) + focus: nothing mounted', nothing_mounted(st)))
        checks.append(('Community (soft) + focus: no mount crumb %s' % cr, not any(mounted_anything(c) for c in cr)))
