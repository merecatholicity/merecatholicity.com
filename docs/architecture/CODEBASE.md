...
| 4 | **Middleware + declarative routes + file-split** — the 91-branch chain is a declarative `ROUTES` table (route-parity diff = identical); `keyed`/`keyedGated` middleware on the byte-exact handlers; and the worker is **split**: `lib.ts` (2.4k lines — the shared core: constants, crypto, auth, DB/notification/broadcast helpers) is now separate from `index.ts` (handlers + Durable Objects + dispatch, 6.4k→4.2k lines). Split behavior-proven: tsc-clean + the wrangler bundle's code line-set is unchanged bar cosmetic esbuild renames. (Remaining: `routes/*`·`durable/*` handler-group extraction.) | index.ts −2.2k lines; declarative dispatch; core/handler seam | ◑ |
