...
- **Cache-busting `comments.js`.** The client is included as `comments.js?v=NN` in every commented page (currently 9 includes); bump `NN` with one `sed` across all of them on any `comments.js` change, or Cloudflare serves the stale script. Cloudflare caches each `?v=N` URL independently and treats it as immutable, so **do not fetch a freshly-bumped `?v=N` URL until GitHub Pages has finished deploying the new file** — a probe mid-deploy freezes the OLD bytes under the new key, and the only clean recovery is to bump again to an unpolluted number. To test origin freshness without polluting a real key, fetch a throwaway query string (`comments.js?probe123`), which always misses cache and hits origin. `style.css` is unversioned, so CSS changes propagate on Cloudflare's cache TTL rather than instantly.
...
- **Moderation is all in-platform** (the comments worker sends no email at all now; the old signed-link moderation emails are gone, and its only remaining secret is `TURNSTILE_SECRET`). Admin powers live in the per-comment user-fingerprint dropdown: **lock/unlock** an identity (reversible; a locked hash is logged out and refused every keyed call), **delete user and all posts** (soft-deletes their comments, removes profile+avatar, and locks the hash), and **ban/unban the shown IP**. A `blockedReason()` gate on every keyed endpoint (writes + the DM unread poll) returns `{blocked}`, and the client logs the user out and redirects to `terms.html` where `flash.js` shows a non-fading banner. Public/cached reads are never gated. Two D1 tables: `locks` (hash) and `ip_bans` (raw IP). Admin-only pages: **Activity audit** (now with a **Pending review** queue that Approves/Deletes AI-held comments — the replacement for the old email approve link) and **IP ban list** (`?ipbans=1`, manual add/remove). The legacy `bans` table is still checked but no longer written. Each topic on a category listing also carries admin **lock/unlock**, **delete**, and **pin/unpin (sticky)** links via the `/moderate` endpoint (act `sticky`/`unsticky`); a pinned topic sorts above the rest of its category (a `sticky` column on `comments`, `ORDER BY COALESCE(sticky,0) DESC, last DESC`) and wears a maroon `(sticky)` mark in the list and at its head.
- **Direct messages** ride the same worker: strictly 1v1 threads in D1 (`dm_threads`/`dms`/`dm_blocks`, canonical-pair unique row, denormalized unread state), POST-with-key only (private; no admin visibility by design), Turnstile-gated sends, and shadow-blocks: a blocked sender's messages read as delivered to them but are stored `held`, invisible to the recipient, and an unblock releases them all with their original timestamps. Client: Inbox at `community.html?inbox=1`, threads at `?dm=<hash>`, unread badge by the Logout links throttled by a 90-second localStorage cache, and a member autocomplete fed by one cacheable `/dm/directory` fetch with all fuzzy matching done client-side.
