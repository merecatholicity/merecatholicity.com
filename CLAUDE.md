...
- **A call rings the phone, and only a missed call is a notification** (2026-09-12). The offer
  ...
  web; the notification is the swipe). A late answer re-sends the caller's ICE. The 'call' bell,
  the thread's `call:missed` line and the "Missed call" push are written ONCE, by the caller's
