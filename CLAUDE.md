...
- **Moderation is all in-platform** (the comments worker sends no email at all now; the old signed-link moderation emails are gone, and its only remaining secret is `TURNSTILE_SECRET`). Admin powers live in the per-comment user-fingerprint dropdown: **lock/unlock** an identity (reversible; a locked hash is logged out and refused every keyed call), **delete user and all posts** (soft-deletes their comments, removes profile+avatar, and locks the hash), and **ban/unban the shown IP**. A `blockedReason()` gate on every keyed endpoint (writes + the DM unread poll) returns `{blocked}`, and the client logs the user out and redirects to `terms.html` where `flash.js` shows a non-fading banner. Public/cached reads are never gated. Two D1 tables: `locks` (hash) and `ip_bans` (raw IP). Admin-only pages: **Activity audit** (now with a **Pending review** queue that Approves/Deletes AI-held comments — the replacement for the old email approve link) and **IP ban list** (`?ipbans=1`, manual add/remove). The legacy `bans` table is still checked but no longer written.
