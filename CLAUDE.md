...
`purescript/src/Domain/*.purs` (the kernel, 32 modules), `comments-worker/`, `contact-worker/`,
...
`mcRich`). The worker (`comments-worker/src/index.ts` the `ROUTES` table, `routes/*.ts` the handlers
