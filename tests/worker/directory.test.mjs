...
test('a keyed read registers the identity once, through the one road, and spares it — but does not list it', async () => {
  const db = freshDb();
  ...
  assert.equal(await isEstablished(env, dan.hash), true, 'and it is spared the challenge, as before');
