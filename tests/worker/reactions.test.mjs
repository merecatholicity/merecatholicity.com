...
test('the DM reaction rings the other side, only for THEIR message; the row is read with its sender', () => {
  ...
  assert.ok(/SELECT m\.id, m\.thread_id, m\.sender_hash, COALESCE\(m\.redacted, 0\) AS redacted, t\.kind FROM dms m/.test(h));
  assert.ok(/if \(row\.sender_hash !== me\) \{/.test(h), 'my own message rings nothing; any other member\'s rings theirs (0016)');
  assert.ok(/kind: 'dm-react', topicId: Number\(row\.thread_id\), commentId: id/.test(h) && /if \(!onScreen\) \{ const ring = notifyReact\(env, bell\);/.test(h) && /else await retractReactNotif\(env, bell\);/.test(h),
