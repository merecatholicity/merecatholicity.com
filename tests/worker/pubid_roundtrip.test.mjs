 ...
 * endpoint must find the member the account hash names. */
...
test('a pubid is not the account hash, and resolveId inverts it', async () => {
  const db = freshDb();
  const e = env(db);
  ...
  clearIdCaches();
  /* resolveId reads the stored map; serveId (via any egress) fills it — here we
     seed it as the backfill would */
  db.prepare('UPDATE profiles SET pubid = ?2 WHERE hash = ?1').run(ann.hash, pid);
...
test('GET /profile?hash=<pubid> finds the member and echoes the pubid, never the account hash', async () => {
  ...
  const e = env(db);
  establish(db, ann.hash);
  db.prepare("UPDATE profiles SET nick = 'Ann', pubid = ?2 WHERE hash = ?1").run(ann.hash, await pub(ann.hash));
  const pid = await pub(ann.hash);
...
test('a DM addressed to a pubid reaches the account the hash names', async () => {
  const db = freshDb();
  const e = env(db);
  for (const who of [ann, bob]) { establish(db, who.hash); publishKey(db, who.hash); db.prepare('UPDATE profiles SET pubid = ?2 WHERE hash = ?1').run(who.hash, await pub(who.hash)); }
...
test('GET /board/author?hash=<pubid> returns that member\'s posts', async () => {
  ...
  const e = env(db);
  establish(db, ann.hash);
  db.prepare('UPDATE profiles SET pubid = ?2 WHERE hash = ?1').run(ann.hash, await pub(ann.hash));
