...
test('the thread draws every call line by side through the membrane, muted, never a bubble', () => {
  ...
  /* `mineMsg(msg)` since 2026-09-19: the server marks the reader's own words
     (`mine`), so the side a line takes is no longer inferred from an id. */
  assert.ok(/if \(mineMsg\(msg\)\) \{[\s\S]*?if \(Number\(msg\.enc \|\| 0\) !== 2\) return;[\s\S]*?placeMsg\(msg\);[\s\S]*?return;\s*\}/.test(app),
