...
let tx = (url, init) => fetch(url, init).then((r) => r);
let keyFn = () => '';
let freshFn = () => false;
...
function q(sep) { return keyFn() && freshFn() ? sep + 'fresh=1' : ''; }

/* a cached GET (through the store) */
function get(url, ttl) {
...
function postRead(path, body, ttl) {
...
function write(base, path, body, invalidate) {
  return Promise.resolve(tx(base + path, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  })).then((r) => r.json()).then((d) => {
    if (d && d.ok && invalidate) (invalidate.length ? invalidate : [null])
      .forEach((p) => store.invalidate(p == null ? undefined : p));
    return d;
  });
}

/* ---- board & comments (reads) ---- */
export const pageComments = (path) => get(API + '?page=' + encodeURIComponent(path) + q('&'), 45000);
export const boardIndex = () => get(API + '/board' + q('?'), 45000);
export const category = (cat, p) => get(API + '/board/cat?cat=' + cat + '&p=' + (p || 1) + q('&'), 45000);
export const topic = (id, extra) => get(API + '/board/topic?id=' + id + (extra || '') + q('&'), 30000);
export const authorPosts = (hash, p) => get(API + '/board/author?hash=' + hash + '&p=' + (p || 1) + q('&'), 45000);
export const search = (qs) => get(API + '/search?' + qs + q('&'), 30000);
export const profile = (hash) => get(API + '/profile?hash=' + hash + q('&'), 30000);
export const directory = () => get(API + '/dm/directory' + q('?'), 45000);
export const backroomCat = (p) => postRead('/board/admin', { key: keyFn(), p: p || 1 }, 45000);
export const backroomTopic = (id, p, find) => postRead('/board/admin', { key: keyFn(), id, p, find }, 30000);

/* ---- board & comments (writes) ---- */
export const post = (payload) => write(API, '', { ...payload, key: keyFn() }, ['']);   // full invalidate: counts everywhere shift
export const edit = (id, body, token) => write(API, '/edit', { id, body, token, key: keyFn() }, ['']);
export const remove = (id) => write(API, '/delete', { id, key: keyFn() }, ['']);
export const report = (id, reason) => write(API, '/report', { id, reason, key: keyFn() }, []);
export const watch = (topicId, act) => write(API, '/watch', { topic: topicId, act, key: keyFn() }, []);
export const markRead = (topicId) => write(API, '/board/read', { topic: topicId, key: keyFn() }, [API + '/board']);
export const markAllRead = () => write(API, '/board/read-all', { key: keyFn() }, [API + '/board']);

/* ---- DMs & notifications ---- */
export const dmThreads = (p) => postRead('/dm/threads', { key: keyFn(), p: p || 1 }, 20000);
export const dmThread = (other, p) => postRead('/dm/thread', { key: keyFn(), with: other, ...(p ? { p } : {}) }, 15000);
export const dmSend = (to, body, token) => write(API, '/dm/send', { to, body, token, key: keyFn() }, [API + '/dm']);
export const dmBlock = (hash, blocked) => write(API, '/dm/block', { hash, blocked, key: keyFn() }, [API + '/dm']);
export const dmDelete = (other) => write(API, '/dm/delete', { with: other, key: keyFn() }, [API + '/dm']);
export const notifications = (p) => postRead('/notifications', { key: keyFn(), p: p || 1 }, 15000);
export const notificationsRead = () => write(API, '/notifications/read', { key: keyFn() }, [API + '/notifications']);

/* ---- profiles ---- */
export const saveProfile = (fields) => write(API, '/profile', { ...fields, key: keyFn() }, [API + '/profile']);
...
export const merecatChat = (id) => write(MERECAT, '/chat', { key: keyFn(), id }, []);
export const merecatForward = (chat, msg, topicId) => write(MERECAT, '/forward', { key: keyFn(), chat, msg, topic: topicId }, ['']);
