...
The \emph{Ethics} of Aristotle is one half of a single treatise of which his
\emph{Politics} is the other half. Both deal with one and the same subject.
...
self-contained. The proem to the \emph{Ethics} is an introduction to the whole
subject, not merely to the first part; the last chapter of the \emph{Ethics}
points forward to the \emph{Politics}, and sketches for that part of the
...
applied: the plural forms, which survive so oddly in English (Ethic\emph{s},
Politic\emph{s}), were intended to indicate the treatment within a single work
of a \emph{group} of connected questions. The unity of the first group arises
...
regard the \emph{Ethics} as dealing with one group of problems and the
\emph{Politics} with a second, both falling within the wide compass of
...
is given by Aristotle himself, where in the last chapter of the \emph{Ethics}
he is paving the way for the \emph{Politics}. In the \emph{Ethics} he has not
...
necessary complement to the \emph{Ethics, i.e.}, a treatise devoted to the
...
\emph{Ethics} and the \emph{Politics} rest upon any such distinction. The
...
\emph{Ethics}, where his attention is directed upon the formation of character,
...
This is the kernel of the \emph{Ethics}, and all the rest is subordinate to
...
treatment of the whole subject covered by the \emph{Ethics} and the
\emph{Politics} together. It sets forth the purpose of the enquiry, describes
...
The \emph{Ethics} is addressed to students who are presumed both to have enough
...
central topic of the \emph{Ethics}, the contents of Book I., cc. iv.-xii. may
...
\emph{single} end, and that that end is knowable or definable in advance of its
...
as something which \emph{ought} to be. The presentation of the Moral Ideal as
...
\emph{Utilitarianism}. Mill’s is much the less consistent: at times he
...
distinctively or peculiarly human, are “rational,” \emph{i e},
...
basis of the best, \emph{i e}, the most desirable life. They form the chief or
central subject-matter of the \emph{Ethics}.
...
life. Such obedience requires knowledge of the rule and acceptance of it \emph{as
...
translated “reason,” means in almost all cases in the \emph{Ethics}
...
mere natural aptitude or endowment, it is a permanent \emph{state} of the
...
yet \emph{to him} without rational justification and without system or unity.
...
difference between acts done voluntarily and acts done of \emph{set} choice or
...
\emph{are} him in action, and the most significant evidence of what he is.
...
to students of the \emph{Ethics}. Almost more than any other part of the work
...
theoretical or practical, goodness of intellect has \emph{two} supreme
...
treatise, consisting of the \emph{Ethics} and the \emph{Politics}, is an
...
can at need justify it by exhibiting its rationale, \emph{i.e.} , its
...
process presupposes \emph{(a)} a clear and just apprehension of the nature of
that end—such as the \emph{Ethics} itself endeavours to supply;
\emph{(b)} a correct perception of the conditions of action, \emph{(a)} at
...
beyond and above the practical ideal of what is best \emph{for man} begins to
...
treatment is smoother and more orderly than elsewhere in the \emph{Ethics}. The
...
This conception of the best life is not forgotten in the \emph{Politics} The
...
with which he opened the \emph{Ethics} would serve as a guide to a father
...
the programme of the \emph{Politics} in the concluding sentence of the
\emph{Ethics} His ultimate object is to answer the questions, What is the best
...
Aristotle’s \emph{Ethics}. The work still remains one of the best
...
\xsection{The following is a list of the works of Aristotle:—}
...
Later editions of separate works \emph{De Anima} Torstrik, 1862, Trendelenburg,
2nd edition, 1877, with English translation, L Wallace, 1882, Biehl, 1884,
1896, with English, R D Hicks, 1907 \emph{Ethica} J S Brewer (Nicomachean),
...
\emph{Historia Animalium} Schneider, 1812, Aubert and Wimmer, 1860; Dittmeyer,
1907

\emph{Metaphysica} Schwegler, 1848, W Christ, 1899

\emph{Organon} Waitz, 1844 6

\emph{Poetica} Vahlen, 1867, 1874, with Notes by E Moore, 1875, with English
...
\emph{De Republica Athenientium} Text and facsimile of Papyrus, F G Kenyon,
...
\emph{Politica} Susemihl, 1872, with German, 1878, 3rd edition, 1882, Susemihl
and Hicks, 1894, etc, O Immisch, 1909

\emph{Physica} C Prantl, 1879

\emph{Rhetorica} Stahr, 1862, Sprengel (with Latin text), 1867, Cope and
...
fruitless), this plainly must be the Chief Good, \emph{i.e.} the best thing of
all.
...
πολιτικὴ\footnote{For this term, as here employed, our language contains no equivalent expression
...
\emph{The Good} of Man. And grant that this is the same to the individual and
...
\emph{it}, and in a general way the man of general mental
cultivation.\footnote{Matters of which a man is to judge either belong to some definite art
...
rises.\footnote{In the last chapter of the third book of this treatise it is said of
...
and reasoning to principles:\footnote{Ἀρχὴ is a word used in this treatise in various
...
the judges to the further end, or \emph{vice versâ}.

Of course, we must begin with what is known; but then this is of two kinds,
what we \emph{do} know, and what we \emph{may} know:\footnote{The difficulty of this passage consists in determining the signification of the
...
respectively what \emph{is} and what \emph{may} be known. All truth is [Greek:
gnorimon aplos], but that alone [Greek: aemin] which we individually realise,
therefore those principles alone are [Greek: gnorima aemin] which \emph{we have
...
perhaps then as individuals we must begin with what we \emph{do} know. Hence
...
\xsection{Chapter III.}

But to return from this digression.

Now of the Chief Good (\emph{i.e.} of Happiness) men seem to form their notions
...
good:\footnote{Or “prove themselves good,” as in the Prior Analytics, ii 25,
...
chairousi de oti eisin agathoi, pisteuontes te ton legonton krisei]} for
...
one would call happy, except for mere disputation’s sake.\footnote{[Greek: thesis] meant originally some paradoxical statement by any philosopher
...
sufficient length in my Encyclia.\footnote{A lost work, supposed to have been so called, because containing miscellaneous
questions.}

A third line of life is that of contemplation, concerning which we shall make
our examination in the following pages.\footnote{It is only quite at the close of the treatise that Aristotle refers to this,
...
been wasted on them.\footnote{Or, as some think, “many theories have been founded on them.”}
...
these εἴδη.\footnote{The ἰδέα is the archetype, the
...
which exists of itself, \emph{i.e.} Substance, is prior in the nature of things
...
bads:\footnote{The list ran thus—}
which philosophers, in fact, Speusippus\footnote{Plato’s sister’s son.}
...
is so.\footnote{This is the capital defect in Aristotle’s eyes, who being eminently
...
The same result\footnote{\emph{i.e.} the identification of Happiness with the Chief Good.}
...
not being reckoned with any other thing,\footnote{\emph{i.e.} without the capability of addition.}
...
before:\footnote{And then Happiness would at once be shown not to be the Chief Good. It is a
...
distinct from all these, has some work of his own?\footnote{Compare Bishop Butler’s account of “Human Nature as a System”
...
also spoken of in two ways,\footnote{\emph{i.e.} as working or as quiescent.}
...
τελείῳ;\footnote{The mere translation of this term would convey no idea of its meaning, I have
...
And again, you must not demand the reason either in all things alike,\footnote{This principle is more fully stated, with illustrations, in the Topics, I.
chap. ix.}
...
\emph{i.e.} starting-point or principle.

And of these first principles some are obtained by induction, some by
perception,\footnote{Either that of the bodily senses, or that of the moral senses. “Fire
...
working. For the state or habit\footnote{I have thought it worthwhile to vary the interpretation of this word, because
...
and the good, it is they who act who rightly win the prizes.\footnote{Another and perhaps more obvious method of rendering this passage is to apply
...
is not a good man at all who feels no pleasure in noble actions,\footnote{“Goodness always implies the love of itself, an affection to
...
Still\footnote{In spite of theory, we know as a matter of fact that external circumstances are
...
properly to an investigation different from ours:\footnote{It is remarkable how Aristotle here again shelves what he considers an
...
maimed\footnote{This term is important, what has been maimed was once perfect; he does not
...
these facts.\footnote{[Greek] Plato. Phædon. xlvi.}
...
instruments.\footnote{But why give materials and instruments, if there is no work to do?}
...
of relations to the ancestors.\footnote{The supposed pair of ancestors.}
...
But we must revert to the point first raised,\footnote{Solon says, “Call no man happy till he is dead.” He must mean
either, The man when dead \emph{is} happy (a), or, The man when dead \emph{may be
said to have been happy} (b). If the former, does he mean positive happiness
(a)? or only freedom from unhappiness (β)? \emph{We} cannot allow (a),
...
when he \emph{is} happy the truth is not to be asserted of him, because we are
...
incidentally to the truth of our account of Happiness.\footnote{The difficulty was raised by the clashing of a notion commonly held, and a fact
...
than the sciences: and of these last again\footnote{I have taken τούτον
αὐτῶν to refer to
...
the drama of life:\footnote{The term seems to be employed advisedly. The Choragus, of course, dressed his
actors \emph{for their parts;} not according to their fancies or his own.
...
blessed \emph{as Men}.\footnote{Always bearing in mind that man “never continueth in one stay.”}
...
are.\footnote{The meaning is this: personal fortunes, we have said, must be in certain weight
...
to that of faculties\footnote{This is meant for an exhaustive division of goods, which are either so \emph{in
esse} or \emph{in posse}.

If \emph{in esse}, they are either above praise, or subjects of praise.
Those \emph{in posse}, here called faculties, are good only when rightly used.
...
ludicrous aspect\footnote{The doubt is, whether [Greek] or [Greek] is the subject of the sentence. It is
...
Eudoxus\footnote{Eudoxus, a philosopher holding the doctrine afterwards adopted by Epicurus
...
mind.\footnote{See the Rhetoric, Book I. chap ix.}
...
Well, we are to enquire concerning Excellence, \emph{i.e.} Human Excellence of
...
of the soul we do not. But, it may be, not the less\footnote{The unseen is at least as real as the seen.}
...
\emph{how} it is different, that is irrelevant.)
...
self-mastery,\footnote{The terms are borrowed from the Seventh Book and are here used in their strict
...
λόγος of mathematics.)\footnote{This is untranslateable. As the Greek phrase, [Greek: echein logon tinos],
...
furniture, \emph{i.e.} knowing them.}
...
mental state;\footnote{[Greek: xin] may be taken as opposed to [Greek: energeian], and the meaning
...
Well: human Excellence is of two kinds, Intellectual and Moral:\footnote{Which we call simply virtue.}
now the Intellectual springs originally, and is increased subsequently, from
teaching (for the most part that is\footnote{For nature must of course supply the capacity.}),
...
by nature, nor in despite of nature,\footnote{Or “as a simple result of nature.”}
...
virtues.\footnote{This is done in the Sixth Book.}
...
particular cases is exactness attainable:\footnote{It is, in truth, in the application of rules to particular details of practice
...
pain; whereas he who does it with pain is not brave.\footnote{The words ἀκόλαστος and
...
entirely. So a certain degree of fear is necessary to the \emph{formation} of
...
Virtues as certain states of impassibility and utter quietude,\footnote{Virtue consists in the due regulation of \emph{all} the parts of our nature our
...
course when the fruit was formed. Of them we might truly say, \emph{Solitudinem
...
produces something grammatical but does so grammarian-wise, \emph{i.e.} in
...
The facts,\footnote{I have adopted this word from our old writers, because our word \emph{act} is
so commonly interchanged with \emph{action}. [Greek: Praxis] (action) properly
...
[Greek: praxis] may be \emph{good} though the [Greek: pragma] be wrong, as if a
...
[Greek: praxis] \emph{right}. No repetition of [Greek: pragmata] goes to form a
...
Next, we must examine what Virtue is.\footnote{Being about to give a strict logical definition of Virtue, Aristotle
...
blamed,\footnote{That is, not for \emph{merely having} them, because we did not make
ourselves.
...
In all quantity then, whether continuous or discrete,\footnote{This refers to the division of quantity ([Greek: poson]) in the Categories.
...
is a mean between excess and defect. Now by the mean of the thing, \emph{i.e.}
...
larger, and this mean is according to arithmetical proportion.\footnote{Numbers are in arithmetical proportion (more usually called progression), when
they increase or decrease by a common difference thus, 2, 6, 10 are so, because
2 + 4 = 6, 6 + 4= 10, or \emph{vice versa}, 10 - 4 = 6, 6 - 4 = 2.}

But the mean relatively to ourselves must not be so found ; for it does not
follow, supposing ten minæ\footnote{If the mina be taken at 15 oz. avoirdupois, (Dict. of G. and R. Antiquities,
article \emph{Talentum},) we must be sadly degenerate in our gastric capacity.}
...
It is moral excellence, \emph{i.e.} Virtue, of course which I mean, because
...
relative mean, determined by reason, and\footnote{The two are necessary, because since the reason itself may be perverted, a man
...
We must take these details then from the well-known scheme.\footnote{This is one of the many expressions which seem to imply that this treatise is
rather a collection of notes of a \emph{vivâ voce} lecture than a set formal
...
III. In respect of giving and taking wealth\footnote{Afterwards defined as “All things whose value is measured by
money.”}
(\emph{a}):
...
χαυνότης,\footnote{We have no term exactly equivalent; it may be illustrated by Horace’s use
of the term \emph{hiatus}:
...
this is a case of [Greek: chaunotes] or fruitless and unmeaning \emph{hiatus};
...
the virtuous character sometimes by the name ambitious,\footnote{In like manner \emph{we} talk of laudable ambition, implying of course there
...
with shame on all and any occasions: the man who is in the defect, \emph{i.e.}
who has no shame at all in his composition, is called Shameless: but the right
character Shamefaced.

Indignation against successful vice,\footnote{An expression of Bishop Butler’s, which corresponds exactly to the
...
feelings or actions, and \emph{vice versa}. The brave man, for instance, shows
...
from one another\footnote{That is, in the same genus; to be contraries, things must be
...
must take the least of the evils as the safest plan;\footnote{“[Greek: Deuteros plous] is a proverb,” says the Scholiast on the
...
timber.\footnote{That is, you must allow for the \emph{recoil}. “Naturam expellas furca
...
from us, we shall err the less.\footnote{This illustration sets in so clear a light the doctrines entertained
...
Moral Sense.\footnote{Αἴσθησις is here used as an analogous
...
disgrace or pain to secure great and honourable equivalents; if \emph{vice
...
instead of to his own capacity for being easily caught by them;\footnote{A man is not responsible for being [Greek: theratos], because “particular
...
action, cannot be said to have done it \emph{with} his will because he did not
know he was doing it, nor again \emph{against} his will because he is not sorry
for it.
...
Again, there seems to be a difference between acting \emph{because of}
ignorance and acting \emph{with} ignorance: for instance, we do not usually
...
his own true interest;\footnote{Virtue is not only the duty, but (by the laws of the Moral Government of the
...
because ignorance which affects moral choice\footnote{Any ignorance of particular facts affects the rightness not of the [Greek:
praxis], but of the [Greek: pragma], but ignorance of \emph{i.e.} incapacity to
discern, Principles, shows the Moral Constitution to have been depraved,
\emph{i.e.} shows Conscience to be perverted, or the sight of Self-love to be
impaired.}
...
what, sometimes with what, as with what instrument, and with what result;\footnote{[Greek: eneka] primarily denotes the relation of cause and effect
...
\emph{designed} causation and so [Greek: on eneka] is used for Motive, or final
cause.
...
eneka] that is to say, the King’s death was \emph{in fact the result},
...
we are bound to grasp at: now there are occasions on which anger is a duty,\footnote{Bishop Butler would agree to this: he says of settled deliberate anger,
...
and there are things which we are bound to lust after,\footnote{Aristotle has, I venture to think, rather quibbled here, by using [Greek:
...
Again: how does the involuntariness make any difference\footnote{The meaning is, that the \emph{onus probandi} is thrown upon the person who
maintains the distinction, Aristotle has a \emph{prima facie} case. The whole
...
as a separate argument but if so, the argument would be a mere \emph{petitio
...
not the same with opinion of any kind,\footnote{\emph{i.e.} on objects of Moral Choice, opinion of this kind is not the same as
Moral Choice, because actions alone form habits and constitute character,
opinions are in general \emph{signs} of character, but when they begin to be
...
still uncertain how in any given case they will issue, \emph{i.e.} in which
...
sometimes how or through what.\footnote{The introduction of the words [Greek: dia tinos] seems a mere useless
...
off and definite,\footnote{Suppose that three alternatives lay before a man, each of the three is of
...
shall do a thing when he has brought the origination up to himself, \emph{i.e}.
to the governing principle in himself,\footnote{Compare Bishop Butler’s “System of Human Nature,” in the
...
grasp by our Will in accordance with the result of our deliberation.\footnote{These words, [Greek: ek tou bouleusasthai—bouleusin], contain the account
...
each individual whatever impresses his mind with the notion of good.\footnote{Compare the statement in the Rhetoric, 1 10, [Greek: esti d hae men boulaeis
...
the way of Moral Choice, \emph{i.e.} voluntary: but the acts of working out the
...
power to forbear doing, and \emph{vice versâ}: therefore if the doing (being in
a given case creditable) is in our power, so too is the forbearing (which is in
the same case discreditable), and \emph{vice versâ}.
...
as are not in our own power, \emph{i.e.} not voluntary, no one thinks of
...
dissolutely does not \emph{wish} to attain the habits of these vices: for if a
...
out of his hand cannot recall it,\footnote{A stone once set in motion cannot be recalled, because it is then placed under
...
cooperate\footnote{Habits being formed by acting in a certain way under certain circumstances we
...
Yet we do apply the term\footnote{“\emph{Moral} Courage” is our phrase.}
...
also the End, since the character of each is determined by the End.\footnote{The meaning of this passage can scarcely be conveyed except by a
paraphrase.
...
Courage, \emph{i.e.} Courage itself. But Courage is honourable (which implies
...
dispositions, differing in five ways,\footnote{For true Courage is required, i. Exact appreciation of danger. 2. A Proper
...
Those commanders who station staunch troops by doubtful ones,\footnote{This may merely mean, “who give strict orders” not to flinch, which
...
knowledge.\footnote{See Book VI. chap. xiii. near the end [Greek: sokrataes aehen oun logous tas
...
seem, many things in war of no real importance\footnote{Such as the noise, the rapid movements, and apparent confusion which to an
inexperienced eye and ear would be alarming. So Livy says of the Gauls, v. 37,
Nata in \emph{vanos} tumultus gens.}
...
Hermæum),\footnote{In Coronea in Bœotia, on the occasion of the citadel being betrayed to some
...
error they took to flight,\footnote{By the difference of tense it seems Aristotle has mixed up two things,
...
except incidentally:\footnote{The meaning of the phrase [Greek: kata sumbebaekos], as here used, in given in
...
touch of the man destitute of self-control is not indifferently of \emph{any}
...
mean: he has, you see, already the \emph{facts} of the Liberal man, he gives
...
\emph{without} any special motive, he is like a base man because he would not
otherwise have taken pleasure in falsehood, but he shows as a fool rather than
as a knave. But if a man does this \emph{with} a special motive, suppose for
...
it, and wish it:\footnote{Each term is important to make up the character of Justice, men must have the
...
is seen, and \emph{vice versâ}. If, for instance, the good state is firmness of
...
It follows moreover in general,\footnote{But not always. [Greek: Philein], for instance, has two senses, “to
...
and minute,\footnote{\emph{Things} are [Greek: homonuma] which have only their name in common, being
in themselves different. The [Greek: homonumia] is \emph{close} therefore when
...
but not always to the individual.\footnote{See Book I. chap. 1. [Greek: toiautaen de tina planaen echei kai tagatha
k.t.l.]}
...
\emph{i.e.} one who strives for more good than fairly falls to his share: of
...
only in his own person,\footnote{A man habitually drunk in private is viewed by our law as confining his vice to
...
actually gets some advantage by it,\footnote{See the account of Sejanus and Livia. Tac. Annal. IV. 3.}
...
in the way of the unlawful. And as the unequal and the more\footnote{Cardwell’s text, which here gives [Greek: paranomon], yields a much
...
universal Injustice the particular \emph{i.e.} as whole to part.
...
man and a good citizen in every case.\footnote{There are two reasons why the characters are not necessarily coincident. He is
a good citizen, who does his best to carry out the [Greek: politeia] under
which he lives, but this may be faulty, so therefore \emph{pro tanto} is
he.

Again, it is sufficient, so far as the Community is concerned, that he does
the \emph{facts} of a good man but for the perfection of his own individual
...
persons. The Just then must imply four terms at least, for those\footnote{Terms used for persons.}
...
apply merely to number in the abstract,\footnote{By [Greek:——] is meant numbers themselves, 4, 20, 50, etc, by
...
justly.\footnote{The profits of a mercantile transaction (say £1000) are to be divided between A
...
arithmetical.\footnote{\emph{i.e.} where the ratio is that of equality, thus 2 : 2 :: 40 : 40}
...
of the Just.\footnote{Her Majesty’s “Justices.”}
...
and will exceed the untouched line by that portion.\footnote{I have omitted the next three lines, as they seem to be out of place here, and
...
originally,\footnote{This I believe to be the meaning of the passage but do not pretend to be able
...
authority, he must not only be struck but punished also.\footnote{This is apparently contrary to what was said before, but not really so.
...
χάρις\footnote{χάρις denotes, 1st, a kindly feeling issuing in a
...
his own: if then there be first proportionate equality, and \emph{then} the
...
exceedings of the mean:\footnote{The Shoemaker would get a house while the Builder only had (say) one pair of
shoes, or at all events not so many as he ought to have. Thus the man producing
the least valuable ware would get the most valuable, and \emph{vice
versa}.
...
between the persons, \emph{i.e.} the ratio of equality.
...
Then the proportion will be correct, A:B::C:D; \emph{now} Reciprocation will be
...
neither want one another, they do not exchange at all: whereas they do\footnote{[Greek] Compare a similar use of [Greek]. De Interpretatione, II. 2. [Greek].}
...
Injustice occupies \emph{both} the extremes.
...
greater the acting unjustly towards others.\footnote{Every unjust act embodies [Greek: to adikon], which is a violation of [Greek:
...
on in other cases which might be enumerated.]\footnote{This passage certainly occurs awkwardly here. If attached to the close
...
numerically.\footnote{In a pure democracy men are absolutely, \emph{i.e.} numerically, equal, in
...
which does exist by nature, and that which does not.\footnote{Or, according to Cardwell’s reading ([Greek: kineton ou mentoi pan])
...
the same as [Greek: kinoumenon], \emph{i.e.} “we admit there is no Just
which has not been sometimes disallowed, still,” etc. With
Cardwell’s, [Greek: kineton] will mean “which not only \emph{does}
but naturally \emph{may} vary.”}
...
but not till then, only some unjust thing.\footnote{Murder is unjust by the law of nature, Smuggling by enactment. Therefore any
act which can be referred to either of these heads is an unjust act, or, as
Bishop Butler phrases it, an act \emph{materially} unjust. Thus much may be
...
case, the occasion of anger being a notion of injury.\footnote{“As distinct from pain or loss.” Bishop Butler’s Sermon on
...
the case lies (the one who plotted against the other, \emph{i.e.} the real
aggressor, of course, cannot be ignorant),\footnote{This method of reading the passage is taken from Zell as quoted in
...
and he who commits such acts of injustice is \emph{ipso facto} an unjust
...
their will.\footnote{As when a man is “\emph{justified} at the Grass Market,”
\emph{i.e.} hung.}
...
for no man wishes for that which he does not \emph{think} to be good, and the
...
chargeable with an unjust act who in distribution has \emph{given} the larger
share to one party contrary to the proper rate, or he that \emph{has} the
...
these.\footnote{Where the stock of good is limited, if any individual takes more than his share
...
person:\footnote{The reference is to chap. vii. where it was said that the law views the parties
...
above supposed there is but \emph{one} party.}
...
virtues.\footnote{So in the Politics, 1. 2.

\emph{Hae men gar psuchae tou somatos archei despotikaen archaen, o de nous
...
has Reason tightens or slacks his rope;\footnote{I understand the illustration to be taken from the process of lowering a weight
...
otherwise than they are,\footnote{Called for convenience sake Necessary and Contingent matter.}
...
percipients);\footnote{One man learns Mathematics more easily than another, in common language, \emph{he
has a turn for} Mathematics, \emph{i e} something in his mental conformation
...
to do.\footnote{And therefore the question resolves itself into this, “What is the work
of the Speculative, and what of the Practical, faculty of Reason.” See
the description of \emph{apetae} II. 5.}
...
action.\footnote{\emph{praxis} is here used in its strict and proper meaning.}
...
pursue.\footnote{That is to say, the Will waits upon deliberation in which Reason is the judge;
when the decision is pronounced, the Will must act accordingly.

The question at issue always is, \emph{Is this Good?} because the Will is
only moved by an impression of Good; the Decision then will be always \emph{Aye or
No}, and the mental hand is put forth to grasp in the former case, and
retracted in the later.

So far as what must take place in \emph{every} Moral Action, right or
wrong, the Machinery of the mind being supposed uninjured but to constitute a
good Moral Choice, \emph{i e.}. a good Action, the Reason must have said Aye
when it ought.
...
sets it in motion, not the final cause,)\footnote{See the note on [Greek: Arche] on page 4, l. 30.}
...
belonging to some one:\footnote{The cobbler is at his last, why? to make shoes, which are to clothe the feet of
someone and the price to be paid, \emph{i.e.} the produce of his industry, is
...
is either\footnote{It may be fairly presumed that Aristotle would not thus have varied his phrase
...
Affirmation or Negation, be assumed to be in number five:\footnote{The mind attains truth, either for the sake of truth itself ([Greek: aplos]),
...
with a view to \emph{act} ([Greek: phronesis]), or with a view to \emph{make}
([Greek: techne]).
...
previous knowledge; (a statement you will find in the Analytics also,)\footnote{It is the opening statement of the Post Analytics.}
...
these universals.\footnote{Aristotle in his logical analysis of Induction, Prior. Analytics II. 25,
defines it to be “the proving the inherence of the major term in the
middle (\emph{i.e.} proving the truth of the major premiss in fig. 1) through
...
same place that for this reason we must \emph{conceive} ([Greek: noehin]) the
...
of probable Reasoning, and its validity depends \emph{entirely} upon the
...
\emph{literally} false. It will be found useful to compare on the subject of
Induction \emph{as the term is used by Aristotle}, Analytica Prior. II 25 26
...
syllogistically,” \&c. as in the Analytics:\footnote{The reference is made to the Post Analyt I II and it is impossible to
...
this Book.)}
because a man, strictly and properly speaking, \emph{knows}, when he
...
included one by the other, that is, Doing is not Making, nor Making Doing.\footnote{This is the test of correct logical division, that the \emph{membra dividentia}
shall be opposed, \emph{i.e.} not included the one by the other.}
Now\footnote{The meaning of the [Greek: hepehi] appears to be this: the appeal is made in
...
kind of persons we in common language ascribe it.\footnote{See the parable of the unjust Steward, in which the popular sense of [Greek:
...
definite line,\footnote{Compare the [Greek: aplos] and [Greek: kath’ ekasta pepaideumenos] of
Book I. chap. 1.}
...
preserve is the Notion I have mentioned, \emph{i.e.} of one’s own true
interest.\footnote{The two aspects under which Virtue may be considered as claiming the allegiance
...
constitution naturally supreme and “Conscience and Self-Love, \emph{if we
understand our true happiness}, always lead us the same way.” Bishop
Butler, end of Sermon III.

And again:

“If by \emph{a sense of interest} is meant a practical regard to what
...
them:\footnote{See the note on [Greek: Arche] on page 4, l. 30.
...
9.—[Greek: ae gar arethae kai ae mochthaeria. k.t.l.]—iii. 7 \emph{ad
...
not:\footnote{This is not quite fair. Used in its strict sense, Art does not admit of degrees
...
unwittingly,\footnote{He would be a \emph{better Chymist} who should poison intentionally, than he on
...
accuracy;\footnote{The term Wisdom is used in our English Translation of the Old Testament in the
sense first given to [Greek:——] here. “Then wrought Bezaleel
and Ahohab, and every \emph{wise-hearted man, in whom the Lord put wisdom and
...
speak, Knowledge of the most precious objects, \emph{with a head
on}.\footnote{[Greek:——] and [Greek:——], (in the strict sense, for it
...
πολιτικὴ,\footnote{[Greek:——] is here used in its most extensive sense,
...
composed.\footnote{The faculty concerned with which is [Greek:——].}
...
subordinate.\footnote{In every branch of Moral Action in which Practical Wisdom is employed there
...
do to a master).\footnote{[Greek:——], “our mere Operatives in Public business.”
(Chalmers.)}
...
Deliberative and Judicial.\footnote{Practical Wisdom may be employed either respecting Self, (which is
[Greek:——] proper) or not-Self, \emph{i.e.} either one’s
...
matter of enquiry.\footnote{But where can this be done, if there be no community? see Horace’s
...
coincidence in \emph{fact} of enlightened Self-Love and Benevolence \emph{i.e.}
love of others. Compare also what is said in the first Book of this treatise,
chap. v., about [Greek: autarkeia].}

A corroboration of what I have said is\footnote{More truly “implied,” namely, that Practical Wisdom results from
experience.}
...
By the way, a person might also enquire,\footnote{This observation seems to be introduced, simply because suggested by the last,
...
process of abstraction, but the principles of Science\footnote{An instance of Principles gained [Greek: aisthesei]. (Book 1. chap. viii.)}
...
ultimate issue,\footnote{Particulars are called [Greek: eschata] because they are last arrived at in the
...
kind.\footnote{I prefer the reading [Greek: e phronesis], which gives this sense, “Well,
...
Quick perception of causes\footnote{The definition of it in the Organon (Post Analyt. 1. xxiv.), “a happy
...
out.\footnote{Opinion is a complete, deliberation an incomplete, mental act.}
...
may be said to have deliberated \emph{rightly} in one sense, but will have
...
effect though not through right means,\footnote{The End does not sanctify the Means.}
...
End.\footnote{The meaning is, there is one End including all others; and in this sense
...
End, of which\footnote{The relative [Greek: on] might be referred to [Greek: sumpheron], but that
...
The faculty called γνώμη,\footnote{We have no term which at all approximates to the meaning of this word, much
...
Now all these mental states\footnote{Meaning, of course, all those which relate to Moral Action. [Greek: psronaesis
...
are employed upon the extremes,\footnote{See note, on p. 140.}
...
Intuition, moreover, takes in the extremes at both ends:\footnote{There are cases where we must simply accept or reject without proof: either
...
[Greek: nous], \emph{i.e.} he calls by this name the Faculty which attains
Truth in each.}
...
the Contingent, and the minor Premiss:\footnote{\emph{i.e.} of the [Greek: syllogisimai ton prakton].}
for the minor Premisses are the source of the Final Cause, Universals being
made up out of Particulars.\footnote{See the note on [Greek: Archae] on p. 4,1 30. As a matter of fact and mental
experience the Major Premiss of the Practica Syllogism is wrought into the mind
by repeatedly acting upon the Minor Premiss (\emph{i.e.} by [Greek: ethismos]).}
...
And so\footnote{The \emph{consequentia} is this:
...
relates to these (I mean,\footnote{This is a note to explain [Greek: hygieina] and [Greek: euektika], he gives
...
“If it be urged that \emph{knowing what is} good does not by itself make
a Practically-Wise man but \emph{becoming} good; still this Wisdom will be no
...
In the next place we say that they \emph{do} produce results; that Science
makes Happiness, not as the medical art but as healthiness makes
health:\footnote{Healthiness is the formal cause of health.
...
Next, Man’s work \emph{as Man} is accomplished by virtue of Practical
Wisdom and Moral Virtue, the latter giving the right aim and direction, the
former the right means to its attainment;\footnote{[Greek: phronæsis] is here used in a partial sense to signify the Intellectual,
...
undone.\footnote{This is another case of an observation being thrown in \emph{obiter}, not
...
up,\footnote{See Book II. chap. iii. and V. xiii.}
...
to be able to do and attain whatever conduces to \emph{any} given purpose: now
...
do \emph{anything}; we accordingly call the Practically-Wise Clever, and also
those who can and will do anything.\footnote{The article is supplied at [Greek: panourgous], because the abstract word has
...
analysed have for their Major Premiss,\footnote{See the note on [Greek: Archæ] on page 4, l. 30.}
“since —— is the End and the Chief Good”\footnote{And for the Minor, of course,}
(fill up the blank with just anything you please, for we merely want to exhibit
the Form, so that anything will do), but \emph{how} this blank should be filled
is seen only by the good man: because Vice distorts the moral vision and causes
men to be deceived in respect of practical
principles.\footnote{“Look asquint on the face of truth.” Sir T. Browne, Religio Medici.}
...
dispositions attach to us all somehow by nature: we have dispositions\footnote{The term [Greek: sophronikoi] must be understood as governing the signification
...
different fashion.\footnote{Compare the passage at the commencement of Book X. [Greek: nun de phainontai]
...
without Practical Wisdom.\footnote{It must be remembered, that [Greek: phronæsis] is used throughout this chapter
...
the Virtues were reasoning processes (\emph{i.e.} that they were all instances
...
Well, we suppose that we \emph{know} things (in the strict and proper sense of
...
that the idea intended to be conveyed by the term \emph{knowing} is something
...
demonstrative reasoning must be drawn from premisses \emph{true} and
\emph{first}, and \emph{incapable of syllogistic proof}, and \emph{better
known}, and \emph{prior in order of time}, and \emph{causes of the
...
\emph{True}, they must be, because it is impossible to know that which is not.

\emph{First}, that is indemonstrable, because, if demonstrable, he cannot be
said to \emph{know} them who has no demonstration of them for knowing such
things as are demonstrable is the same as having demonstration of them.

\emph{Causes} they must be, and \emph{better known}, and \emph{prior} in time,
\emph{causes}, because we then know when we are acquainted with the cause, and
\emph{prior}, if causes, and \emph{known beforehand}, not merely comprehended
in idea but known to exist (The terms prior, and better known, bear two senses
for \emph{prior by nature} and \emph{prior relatively to ourselves} are not the
same, nor \emph{better known by nature}, and \emph{better known to us} I mean,
by \emph{prior} and \emph{better known relatively to ourselves}, such things as
...
And by \emph{first}, I mean \emph{principles akin to the conclusion}, for
principle means the same as first And the principle or first step in
demonstration is a proposition incapable of syllogistic proof, \emph{i.e.} one
...
of the premisses, it is necessary not merely to \emph{know before}, but to
\emph{know better than the conclusion}, either all or at least some of, the
...
who \emph{knows}, is to be simply and absolutely infallible.

\xchapter{Book VII}

\xsection{Chapter I.}

Next we must take a different point to start from,\footnote{The account of Virtue and Vice hitherto given represents rather what men \emph{may
be} than what they \emph{are}. In this book we take a practical view of
...
V. Some confound the two characters of the man who has \emph{no} Self-Control,
and the man of \emph{Imperfect Self-Control}, while others distinguish between
them.
...
That he can so fail when \emph{knowing} in the strict sense what is right some
...
passion is quite plain.\footnote{This illustrates the expression, “\emph{Deceits} of the Flesh.”}
...
does,\footnote{Another reading omits the [Greek:——]; the meaning of the whole
passage would be exactly the same—it would then run, “if he had
been convinced of the rightness of what he does, \emph{i.e.} if he were now
...
Rather the following is the account of it: the term \emph{knowing} has two
senses; both the man who does not use his Knowledge, and he who does, are said
to \emph{know}: there will be a difference between a man’s acting
...
action,\footnote{Major and minor Premises of the [Greek:——] [Greek——]}
...
propositions;\footnote{Some necessarily implying knowledge of the particular, others not.}
...
that for a man to \emph{know} in the one sense, and yet act wrongly, would be
...
Empedocles,\footnote{As a modern parallel, take old Trumbull in Scott’s “Red
Gauntlet.”}
...
Opinion:\footnote{That is, as I understand it, either the major or the minor premise, it is true,
that “all that is sweet is pleasant,” it is true also, that
“this is sweet,” what is contrary to Right Reason is the bringing
in this minor to the major \emph{i.e.} the universal maxim, forbidding to
...
physiologists\footnote{As we commonly speak, Metaphysicians. Physiology of course includes
Metaphysics.}
...
so has it that his having does not amount to \emph{knowing} but merely saying,
as a man when drunk might repeat Empedocles’ verses; and because the
minor term\footnote{[Greek: oron]. Aristotle’s own account of this word (Prior Analyt ii. 1)
...
avoids\footnote{The Greek would give “avoids excessive pain,” but this is not true,
...
men are blamed not merely\footnote{Compare Bishop Butler on Particular Propensions, Analogy, Part I chap v sect.
iv.}
...
in the way of analogy,\footnote{That is, they are to the right states as Vice to Virtue.}
...
one’s nails, or eating coals and earth.\footnote{See the letter of Sabina Rentfree. Spectator, 431.}
...
contempt\footnote{Consult in connection with this Chapter the Chapter on [Greek: orgae] in the
...
“Cyprus-born Goddess, \emph{weaver of deceits}”

Or Homer of the girdle called the Cestus,

“Persuasiveness \emph{cheating} e’en the subtlest mind.”
...
insolence.\footnote{The reasoning here being somewhat obscure from the concisement of expression,
...
bodily pains, not because he \emph{cannot}, but because he \emph{chooses not
to}, withstand them.

But of the characters who go wrong without \emph{choosing} so to do, the one is
...
However, of the two characters which have been mentioned,\footnote{[Greek: ton dae lechthenton]. Considerable difference of opinion exists as to
...
The [Greek: akolastos] \emph{deliberately} pursues pleasure and declines
...
so.\footnote{“If we went into a hospital where all were sick or dying, we should think
...
to moral choice and in another the result of it:\footnote{Before the time of trial comes the man deliberately makes his Moral Choice to
...
Is not this the truth, that \emph{incidentally} it is by conclusions and moral
choice of any kind that the one character abides and the other does not, but
\emph{per se} true conclusions and right moral choice:\footnote{Let a man be punctual \emph{on principle} to any one engagement in the day, and
...
to explain what is meant by incidentally, and \emph{per se}; suppose a man
chooses or pursues this thing for the sake of that, he is said to pursue and
choose that \emph{per se}, but this only incidentally. For the term \emph{per
...
Reason,\footnote{Because he is destitute of these minor springs of action, which are intended to
...
\xsection{APPENDIX. Book VII. Chapters 12 to 15. (Bekker.)}
...
process is akin to the end to be attained: \emph{e.g.} no process of building
...
are things pleasant \emph{per se}, in the former even those things which are
...
\emph{simpliciter}: the exercise of the pure Intellect sometimes hurts a
...
We have already said in what sense all Pleasures are good \emph{per se} and in
...
as bad \emph{per se}, partly as being a hindrance in some particular way. Now
the contrary of that which is to be avoided, \emph{quâ} it is to be avoided,
\emph{i.e.} evil, is good. Pleasure then must be \emph{a} good.
...
though most Pleasures be (let us assume) low \emph{per se}.
...
Pain is felt in excess men pursue Pleasure in excess, \emph{i.e.} generally
...
is how men come to be utterly destitute of Self-Mastery, \emph{i.e.} low and
bad.

But those Pleasures which are unconnected with Pains do not admit of excess:
\emph{i.e.} such as belong to objects which are naturally pleasant and not
...
\emph{i.e.} simple: not motion merely but also motionlessness acts, and
...
\emph{i.e.} it is neither simple nor good.
...
(“They \emph{two} together going,” Homer says, you may remember),
...
required;\footnote{“Owe no man anything, but to \emph{love} one another for he that loveth
another \emph{hath fulfilled the Law}.” Romans XIII. 8.}
...
agree.”\footnote{[Greek: kerameis]. The Proverb in full is a line from Hesiod,
...
individual feels Friendship not for what \emph{is} but for that which
\emph{conveys to his mind the impression of being} good to himself. But this
...
safe and sound for his own use and benefit.\footnote{In this sense, therefore, is it sung of Mrs. Gilpin that she}
...
themselves; that is to say, not in so far as the friend beloved \emph{is} but
...
because in this the parties are similar, and so on:\footnote{Cardwell’s reading, [Greek: tautae gar omoioi, kai ta loipa] is here
...
The connection then subsisting between the good is Friendship \emph{par
...
But it \emph{is} possible to please many on the score of advantage and
...
proportionate degree of Friendship.\footnote{The Great man will have a right to look for more Friendship than he bestows,
but the Good man \emph{can} feel Friendship only for, and in proportion to, the
...
incidentally:\footnote{See note on page 68, 1. 8.}
...
nor Justice: nor even towards a horse or an ox, or a slave \emph{quâ} slave,
because there is nothing in common: a slave as such is an animate tool, a tool
an inanimate slave. \emph{Quâ} slave, then, there is no Friendship towards him,
only \emph{quâ} man: for it is thought that there is some principle of Justice
...
creates a sameness with one another;\footnote{See I. Topics, Chap. v. on the various senses of [Greek: tauton].}
...
general:\footnote{“For the mutual society, help, and comfort that the one ought to have of
...
\emph{wish} what is honourable, but, when tested, \emph{choose} what is
...
ought not to make a disinterested friend\footnote{Which one would be assuming he was, if one declined to recognise the obligation
...
the connection than the other, only \emph{not more of the same thing}: but the
...
\emph{would} sever himself utterly from a son, except in a case of exceeding
...
the man who first takes? because, \emph{primâ facie}, the man who first gives
...
\emph{both} shall allow to be proportionate, and, where this cannot be, then
...
repentance,\footnote{“Neither the Son of man, that He should \emph{repent}.” Numbers
xxiii. 19.
...
though actually tearing them asunder.\footnote{“I have sometimes considered in what troublesome case is that Chamberlain
...
in opinion on \emph{any} point, as, for instance, on points of astronomical
...
between \emph{two} persons: whereas they who have many friends, and meet
...
that it is exceedingly bad;\footnote{See note, p. 43.}
...
mean.\footnote{See Book II. chap. ix.}
...
is not the Chief Good:\footnote{See Book I. chap. v. ad finem.}
...
\emph{think} we say \emph{is}. And he who would cut away this ground from under
...
belong to the class of qualities:\footnote{The notion alluded to is that of the [greek: idea]: that there is no real
...
will apply to justice and all the other virtues,\footnote{See note on page 136, 1. 15.}
...
Movements\footnote{Movement is, according to Aristotle, of six kinds:}
...
into that of anger, one cannot be in the state quickly,\footnote{\emph{A} may go to sleep quicker than \emph{B}, but cannot \emph{do more sleep}
in a given time.}
...
not \emph{any}thing is generated from \emph{any}thing, but a thing resolves
...
ophthalmia.\footnote{Compare Book III. chap. vi. [Greek: osper kai epi ton somaton, k. t. l.]}
...
differing in kind, \emph{i.e.} in the sources from which they are derived. Let
...
house-building\footnote{Which is of course a [Greek: genesis].}
...
given portion.\footnote{That is, subordinate Movements are complete before the whole Movement is.}
...
“a whole.”\footnote{Pleasure is so instantaneous a sensation, that it cannot be conceived divisible
...
effect as the Pains proper to it,\footnote{A man is as effectually hindered from taking a walk by the [Greek: allotria
...
Now in all such cases that is held to \emph{be} which impresses the good man
...
those which are thought to be good what kind, or which, must we say is \emph{The
...
specially and properly \emph{The Pleasures of Man;} and all the rest in a
...
this is accompanied by earnestness,\footnote{I have thus rendered [Greek: spoudae (ouk agnoon to hamartanomenon)]; but,
...
to those who are yet seeking for, it.\footnote{Bishop Butler, \emph{contra} (Sermon XV.).
...
rest;\footnote{The clause immediately following indicates that Aristotle felt this statement
...
distinct\footnote{That is to say, he aims at producing not merely a happy aggregate, but an
...
\emph{would} be absurd then for a man not to choose his own life but that of
some other.
...
Again, Practical Wisdom\footnote{See note, page 146, 1. 17.}
...
those who are unjust make a pretence of \emph{wishing} to do justly), and the
...
man’s taking excellent care of some \emph{one} case, though he possesses
