...
client/          The classic client: comments.ts (the boot — page state, the core helpers,
                 ...
                 surface · the six DM factories (dm-crypto · dm-message · dm-pickers · dm-inbox ·
                 dm-thread · dm-styles) · merecat · admin-core · admin.ts per boot; esbuild bundles
                 the eager modules into docs/comments.js (an ES module now) and the lazy three —
                 admin's views, merecat, the DM thread — into content-hashed chunks under
                 docs/chunks/, fetched the first time a route needs them (2026-09-16).
...
npm run build:js   # esbuild  app/shell.ts -> docs/app.js  +  client/comments.ts -> docs/comments.js (+ docs/chunks/* for the lazy modules)
...
`client/<feature>.ts` — the six DM factories, admin-core, and the lazy admin/merecat/dm-thread —
bundled → `docs/comments.js` + `docs/chunks/*`), and both
