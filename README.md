...
forum with direct messages and notifications, and **merecat**, a retrieval-augmented
...
scripts/         Site build tooling: nav.py (menus), content.py (content pages),
...
Makefile         The build entrypoint. nav.yml = the single source for the site menu.
eslint.config.js Lints the worker + every hand-written client script.
```
...
```sh
make menu     # nav.yml -> partials/nav.html, rewrite nav in the hand pages, rebuild content + book
make css      # styles/*.css -> docs/style.css
make bundle   # app/ -> docs/app.js  (esbuild via deno, minified IIFE, deterministic)
```

- **Navigation** has one source, `nav.yml`. `scripts/nav.py` renders `partials/nav.html`
...
| `make menu` | Regenerate nav from `nav.yml`, then rebuild |
...
| Kind | Resource | Use |
| --- | --- | --- |
| **D1** (edge SQLite) | `merecatholicity-comments` | Comments, forum, DMs, notifications, profiles, moderation, FTS5 search index |
