...
styles/          Tailwind CSS source. main.css is the entry (imports the tailwind
                 theme + utilities and the hand-authored NN-*.css); Tailwind builds it
                 into docs/style.css.

app/             The Lit app shell (shell.js + views/ + store.js + api.js + richtext.js),
                 bundled by esbuild into docs/app.js.

vendor/          Vendored, pinned RUNTIME JS (lit-all.min.js). The edge CSP forbids
                 CDN/inline scripts, so runtime libraries are vendored; the BUILD
                 toolchain (Tailwind, esbuild, eslint, wrangler) is managed by npm.
...
package.json     npm project: the dev toolchain (Tailwind, esbuild, eslint, wrangler) and
package-lock.json  the build/lint scripts. Restored per-project with `npm ci`; never global.
```
...
- **Node.js + npm** — install from the system package manager (on Arch, `pacman -S nodejs
  ...
  per-project devDependencies, restored from the committed `package-lock.json` with
  `npm ci`. **Never `sudo npm`, never `npm install -g`**: everything lives in the project's
...
`esbuild` is pinned to an exact version (byte-stable bundles); the rest float within the
