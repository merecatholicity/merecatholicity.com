/* comments-worker/src/alerts.ts — the worker's voice (2026-09-16).
   One function, two channels, both Platform settings: `sendAlert` mails the
...
import { getAppSettings, isDiscordWebhook, sendDiscord } from './lib.ts';
import type { Env } from './env.ts';

export type Alert = { kind: string; subject: string; text: string; embed?: Record<string, unknown> };
export type AlertResult = { email: boolean; discord: boolean; channels: string[]; errors: string[] };
...
/* The channels the stored settings open, as tags — the kernel's rule over the
   ...
   worker's SSRF gate). */
export function alertChannels(settings: Record<string, string>): string[] {
...
export async function sendAlert(env: Env, alert: Alert): Promise<AlertResult> {
  const out: AlertResult = { email: false, discord: false, channels: [], errors: [] };
  ...
  out.channels = alertChannels(settings);
  ...
  console.log(JSON.stringify({
    ...
    kind: alert.kind, channels: out.channels, email: out.email, discord: out.discord, errors: out.errors,
  }));
  return out;
}
