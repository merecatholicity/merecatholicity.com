...
- Any uncaught server error → `500 {ok:false, error:"Server hiccup. Please try
  again shortly."}` — and so does an answer the egress guard refused (2026-09-17:
  a textual answer or live frame that would have carried a worker secret never
  leaves; a client cannot tell the two apart, by design).
...
**The public id is the secret's unsalted digest, and that is a real cost.** One
round of SHA-256 over a guessable key is a wordlist away from the key itself, so
a member who types a passphrase instead of keeping the generated string is only
as safe as that passphrase. Two consequences a client must know: the roster is
keyed (`POST /dm/directory`, 2026-09-18) rather than an anonymous read, and a
...
`/admin/settings`. **The TURN guard (2026-09-17):** the usage chain (23:30 UTC)
reads the month's relayed egress and, at `turn_guard_pct` of the free pool
(default 95), sets `calls_turn` to `0` (`updated_by: turn-guard`) and alerts; the
monthly chain sets it back to `1` on the 1st — only if the guard switched it
off and it is still off (`Domain.Call.turnGuardStep`). Notification kind `'call'` (migration 0009); the pending-call
...
| Route | Body | Returns | Gate |
|---|---|---|---|
| `POST /api/comments/dm/send` | `{key, token, thread_id? \| with?/to?:<64-hex>, body, enc:0\|1\|3, keys?:{hash: sealed}, media_key?}` | `{ok, id, thread_id, created_at}` — a pair's other is named by `with` (the target every other DM road takes) or by `to` (what `/dm/forward`'s items say); **since 2026-09-19 both**, and reading `to` alone had made the first word of a new conversation impossible. **Same shape even when shadow-held** (undetectable to the sender). `404` for a thread you are not in; `409 roster` for a stale key set; a `media_key` already named by a message is accepted only from a member who can read it (a forward). | `POST_LIMIT` · **Turnstile** · gated. Refuses self and the bot on a pair. |
...
| `POST /api/comments/dm/threads` | `{key, p?}` | `{ok, threads:[{thread_id, id, kind, name, other_hash (a pair), nick, avatar, assigned, members:[{hash,nick,avatar,assigned,is_me} ≤4 of the others], member_count, msgs, last_at, unread}], total, unread_total, page, per:20}`. `unread` is the COUNT of the viewer's unread words in that thread, `unread_total` their sum across threads — the tab badge's number. Threads with 0 visible messages, and threads left, are absent. | `READ_LIMIT`, **not** gated. |
| `POST /api/comments/dm/thread` | `{key, thread_id? \| with?, p?, find?}` | `{ok, thread:{id, kind, name, ttl, members:[{hash, nick, avatar, assigned, pubkey, joined_at, left_at, read_at, last_seen, is_me}]}, messages:[{id, sender_hash, body, enc, sealed, created_at, edited_at, opened_at, expires_at, saved, saved_by, redacted, media_key, media_size, media_expired, mine, reactions:[{hash,emoji}]}], total, page, per:20, blocked, unread, unread_from, notif_unread}` — plus, one deploy, the top-level `thread_id`, `ttl`, `other` and a pair message's `react_me`/`react_other`/`liked_*`. `members` lists the departed too (`left_at` set) so their words still open; `read_at` is withheld under that member's receipts "off"; `last_seen` is the hub's stamp as-is. **`is_me` (1 on the reader's own member row) and `mine` (1 on the reader's own messages) are the SERVER'S word on who is reading** (2026-09-19): the key was resolved to answer at all, so a client never infers it by comparing ids — the comparison is what broke when ids changed shape under it. Both are always present, 1 or 0, never absent. **`p` absent → the LAST page.** Opening marks the thread read, starts the disappearing clock (the FIRST open by any recipient starts everyone's), READS every bell the conversation rang you (`dm`, `dm-react`, `call`) and says what was unread BEFORE it did (`unread`, `unread_from`). `find:<message id>` places the answer on that message's page. `404 No such conversation.` for one you are not (or no longer) in. | `READ_LIMIT`, not gated. |
| `POST /api/comments/dm/directory` | `{key}` | `{ok, users:[{hash, joined, nick, assigned}]}` — up to 2000, newest first. Bot and any `merecat…` nick excluded. All fuzzy matching is client-side. **Since 2026-09-16** it lists MEMBERS only — a nick, a live post (forum or feed) or a published DM key; an identity with a bare `profiles` row (what any keyed read leaves behind) is not listed, nor are the test and probe identities (`HIDDEN_HASHES`). **KEYED since 2026-09-18** and no longer cacheable: it was a `GET` any stranger could call, and a member's hash is the unsalted SHA-256 of their key (§2.2), so the whole roster in one anonymous request was a wordlist's shopping list. A missing key is `400 {ok:false}`. | `READ_LIMIT`, **not** gated. |
...
| `POST /api/comments/dm/mykeys` | `{key}` | `{ok, keys:[{msg_id, sealed, sender_pubkey}]}` — every sealed DM key you hold, each with its message's sender pubkey, so a rotation (below) can re-seal them. Your own envelopes only; no account hash rides. | `READ_LIMIT`, gated. |
| `POST /api/comments/profile/rekey` | `{key, newkey, pubkey:<43 b64url>, resealed:{msg_id: <S3-seal>}}` | `{ok, hash}` — **rotate the secret behind your identity** (the P0 chain L3). `key` is the OLD key (proves ownership), `newkey` the new one (must clear the key floor §2.2 and wear nothing yet — else `409`). Every row you own MOVES from your old account hash to the new one in one atomic batch, so your public id and displayed pseudonym CHANGE with it (right for a security rotation — the compromised identity is retired, the writings follow). You must re-seal every DM key first: fetch them (`/dm/mykeys`), open each with the old key, re-wrap `K` to yourself as an `S3.` self-seal under the new key, and pass them as `resealed` — the server **refuses (`409 reseal-incomplete`)** if one is missing, so a rotation never silently drops a conversation. `hash` is your new public id; adopt `newkey` and re-derive on success. | `POST_LIMIT` · gated. |
...
| `POST /api/comments/lock` · `/deleteuser` | `{key,hash,locked}` · `{key,hash}` | Lock/unlock an identity · soft-delete a user + all board posts (their attachments purged), hard-delete their feed — every post of theirs with the comments under it, every comment of theirs elsewhere, the reactions, every media object (2026-09-12) — drop profile/avatar, lock forever. |
