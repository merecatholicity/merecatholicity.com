...
| 2 | `Domain.Profile` + `Domain.Faith` + `Domain.Pseudonym` **shipped**; dmScore / pagerPages / emoji / cats / TTLs planned | in progress | profile caps (admin bio 1000→500 fixed), faith enum, and the displayName pseudonym (ADJ/NOUN wordlists, 20+ call sites) single-sourced; comments.js fallbacks stay (no-bundle); worker `MAX_*`/`FAITHS`/`displayName` single-source here in Phase 6 |
