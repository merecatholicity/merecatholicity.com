...
| 2 | `Domain.Profile` (drift-killer) + `Domain.Faith` **shipped**; dmScore / pagerPages / emoji / cats / TTLs planned | in progress | profile caps single-sourced (admin bio 1000→500 fixed); faith code↔label+order single-sourced (`Core.faithLabel`/`faiths`); comments.js fallbacks stay (no-bundle); worker `MAX_*`/`FAITHS` single-source here in Phase 6 |
