...
| 6 | Worker single-source — **rank + faith + profile caps + pseudonym + DM TTLs + the BIBLE book table retired** (worker imports `Domain.Rank`/`Faith`/`Profile`/`Pseudonym`/`Dm`/`Scripture`; wrangler bundles `../../purescript/output`, `make worker-deploy` runs psbuild first, deployed + live-verified). The worker's *extended* `MERECAT_BIBLE` (Vulgate/deuterocanon) and the `SafeMatch` FTS remain (worker-only, no client dup). | in progress | `rankFor`, `FAITHS`/`FAITH_LABELS`, `MAX_*`, `displayName`, `DM_TTLS`, and the `/config` bible table all read the PureScript modules — the same source the client bundles; **the "two files must stay identical" hazard class is closed for every shared constant** |
