...
| 6 | Worker single-source — **rank + faith + profile caps + pseudonym + DM TTLs retired** (worker imports `Domain.Rank`/`Faith`/`Profile`/`Pseudonym`/`Dm`; wrangler bundles `../../purescript/output`, `make worker-deploy` runs psbuild first). Scripture (BIBLE/MERECAT_BIBLE) + `SafeMatch` FTS still to do. | in progress | the worker's `rankFor`, `FAITHS`/`FAITH_LABELS`, `MAX_*`, `displayName`, `DM_TTLS` all read the PureScript modules — the same source the client bundles; **the "wordlists in two files" hazard class is closed for these** |
