...
| 6 | Worker single-source — **rank + faith + profile caps + pseudonym + DM TTLs + the BIBLE book table + the FTS `SafeMatch` retired** (worker imports `Domain.Rank`/`Faith`/`Profile`/`Pseudonym`/`Dm`/`Scripture`/`Fts`; wrangler bundles `../../purescript/output`, `make worker-deploy` runs psbuild first, deployed + live-verified). The worker's *extended* `MERECAT_BIBLE` (Vulgate/deuterocanon) is the last worker-only leftover (no client dup, low value). | in progress | `rankFor`, `FAITHS`/`FAITH_LABELS`, `MAX_*`, `displayName`, `DM_TTLS`, and the `/config` bible table all read the PureScript modules — the same source the client bundles; **the "two files must stay identical" hazard class is closed for every shared constant.** `Domain.Fts` is the flagship illegal-states slice: a `SafeMatch` has no public constructor, so `buildMatch`/`merecatMatch` (its only producers, worker-only) make FTS5 injection unrepresentable in the type. Tokenization runs the exact JS regexes via a thin FFI (byte-identical `\S`/`\s`/word-class); the sanitizer is pure PS. Parity + injection-safety proven over **304,750 adversarial inputs** (5-lens fuzz: random-Unicode, whitespace-normalization, injection, quotes, caps/stopwords) — 0 mismatches, 0 escapes; the corpus is standing in `run.mjs`. Client bundle byte-identical (the barrel never imports `Fts`), so no `app.js`/`?v` change. |
