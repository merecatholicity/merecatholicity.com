...
| 4 | Application state = State + Events + Transitions (auth ADT, notifications, mute). **In progress** — a 4-concern analysis (auth/identity, post/composer, DM/crypto, notifications/mute) mapped 48 pure decisions vs 44 effects into a slice-ordered plan. **Auth slice 1 shipped:** `Domain.Auth` — the `AuthState` ADT (Anonymous / Authenticating / Pending / Member / Admin, illegal combos unrepresentable) + `isAdmin`/`isMember`/`gate` classification, single-sourcing the `isAdmin()` and `adminGate()` logic (parity 32/32 signal combos, headless-verified). | in progress | `isAdmin()`/`adminGate()` delegate to `window.mcCore.authIsAdmin`/`authGate`; classic logic kept as no-bundle fallback. Remaining auth slices: validKey guard, loadMyProfile fold, and the capstone (`state.auth :: AuthState` via pure transitions). **Also shipped: `Domain.Mute`** (bot-exempt membership + toggle, delegating isMuted/toggleMute) and **`Domain.Blocked`** (the `Locked`/`IpBan`/`Banned` ADT + `messageFor`, single-sourcing blockedOut's two flash strings; the logout/redirect effects stay JS) — both parity + headless verified. Signup/login/logout are NOT Turnstile-gated, so the auth machine is headless-testable; the composer/DM SEND paths need manual-live. |
| 5 | comments.js effect cores (validation/what-to-send in PS; Turnstile/nacl/WS/fetch as FFI); typed API client. **Started:** `Domain.Compose.mentionsIn` — the send-time mention resolution (collectMentions: the @-picks whose token still stands in the body, deduped, in order) is pure PS, delegated from boardPost; the Turnstile/fetch/DOM stay JS. Parity + headless verified. | in progress | The composer SEND envelope + the DM crypto/framing remain JS (Turnstile-gated / crypto-opaque by contract, not headless-testable); `mentionsIn` is the pure "what to send" decision, safe to extract. |
