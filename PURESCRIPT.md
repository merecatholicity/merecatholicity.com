...
| 2 | `Domain.Profile` + `Domain.Faith` + `Domain.Pseudonym` + `Domain.Dm` (TTLs) + `Domain.Pager` (windowing) **shipped**; dmScore / emoji / cats deferred (no dup/bug, lower value) | mostly done | profile caps (admin bio 1000→500 fixed), faith enum, displayName pseudonym (ADJ/NOUN, 20+ sites), DM lifetimes single-sourced; **`Domain.Pager.pagerItems` retires the flagged duplicate** — `util.js pagerPages` and the inline `member.js` reimplementation both now call it (byte-parity 16k cases; the classic `comments.js pageBar` is a deliberately DIFFERENT first-three window, left as-is). comments.js fallbacks stay (no-bundle); worker copies single-source in Phase 6 |
